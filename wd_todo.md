...
- **Multi-room: Locked room type doesn't actually lock (pending)** — see [`design_multi_room.md`](design_multi_room.md). Spectator and Private are confirmed working live (2026-07-16). Locked's room-relock call (`configureRoomForMatch({locked: true})` in `checkHandoffArrival`) runs correctly but BC silently ignores the `Locked` field the same way it ignored `Name` on a plain `ChatRoomAdmin`/`Update` — except unlike the rename, leave+recreate isn't an option here since both players are already in the room by the time it needs to lock. Real fix needs the actual correct mechanism (unknown — no BC client/server source on disk to check against; next step would be inspecting real socket.io traffic from a manual lock via BC's own UI). Code is left in place as-is; room type just doesn't do anything different from Private yet. DW: leaving this for now, Private + Spectator is good enough for today.

- **Detect a player's blocked lock item and warn during negotiation** — idea from DW, spotted live 2026-07-16: a player's synced character data includes `BlockItems` (e.g. MissyMissy's showed `BlockItems.ItemMisc.TimerPasswordPadlock: [""]` after she blocked it manually to test). If a player has the specific item WinnersDice uses for its end-game timer lock blocked, the bot could detect this (same `roomCharacters` data already used by `checkPlayerPermissions`) and warn during negotiation instead of the lock silently failing at end-game time. Not implemented — no code written yet, needs its own design pass (what item(s) to check given `executeEndGame`'s stand-in asset is itself still TBD — see "End game timer/password lock slot" above).
